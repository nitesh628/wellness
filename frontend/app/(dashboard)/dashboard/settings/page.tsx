...
const SettingsPage = () => {
    ...
    const loadUserFromStorage = async () => {
      ...
      if (sessionData && !currentUser) {
        try {
          ...
          if (userDetails) {
            console.log('Loaded user from localStorage:', userDetails)
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      console.log('No current user found in auth slice')
    }
  }, [currentUser])
    ...
    if (settingsData) {
      ...
      if (settingsData.seoSetting && settingsData.seoSetting.basicSetting) {
        setSeoData({
          ...
          siteDescription: settingsData.seoSetting.basicSetting.metaDescription || '',
          siteKeywords: settingsData.seoSetting.basicSetting.keywords?.join(', ') || '',
          ogImage: settingsData.seoSetting.basicSetting.googleAnalyticsId || '',
          twitterHandle: settingsData.seoSetting.basicSetting.twitterHandle || '',
          googleAnalytics: settingsData.seoSetting.basicSetting.googleAnalyticsId || '',
          facebookPixel: '',
          sitemapUrl: 'https://wellnessfuel.com/sitemap.xml',
          robotsTxt: 'https://wellnessfuel.com/robots.txt',
          thirdPartyScripts: {
            googleTagManager: settingsData.seoSetting.thirdPartySetting?.googleTagManagerId || '',
            hotjar: settingsData.seoSetting.thirdPartySetting?.hotjarId || '',
            intercom: settingsData.seoSetting.thirdPartySetting?.intercomAppId || '',
            zendesk: settingsData.seoSetting.thirdPartySetting?.zendeskWidgetKey || '',
            customScripts: settingsData.seoSetting.thirdPartySetting?.customScripts || ''
          },
          metaTags: {
            author: settingsData.seoSetting.metaSetting?.author || '',
            robots: settingsData.seoSetting.metaSetting?.robots || 'index, follow',
            viewport: settingsData.seoSetting.metaSetting?.viewport || 'width=device-width, initial-scale=1',
            themeColor: settingsData.seoSetting.metaSetting?.themeColor || '#10b981',
            customMetaTags: settingsData.seoSetting.metaSetting?.customMetaTags || ''
          }
        })
      }

      // Update Business data
      if (settingsData.businessSetting && settingsData.businessSetting.businessInformation) {
        setBusinessData({
          businessName: settingsData.businessSetting.businessInformation.businessName || '',
          businessEmail: settingsData.businessSetting.businessInformation.businessEmail || '',
          businessPhone: settingsData.businessSetting.businessInformation.businessPhone || '',
          businessAddress: settingsData.businessSetting.businessInformation.businessAddress || '',
          gstNumber: settingsData.businessSetting.businessInformation.gstNumber || '',
          panNumber: settingsData.businessSetting.businessInformation.panNumber || '',
          businessType: settingsData.businessSetting.businessInformation.businessType || 'Private Limited',
          industry: 'Health & Wellness',
          foundedYear: settingsData.businessSetting.businessInformation.foundedYear?.toString() || '2020',
          website: settingsData.businessSetting.businessInformation.website || '',
          socialMedia: {
            facebook: settingsData.businessSetting.socialMediaSetting?.socialMedia?.facebook || '',
            instagram: settingsData.businessSetting.socialMediaSetting?.socialMedia?.instagram || '',
            twitter: settingsData.businessSetting.socialMediaSetting?.socialMedia?.twitter || '',
            linkedin: settingsData.businessSetting.socialMediaSetting?.socialMedia?.linkedin || ''
          }
        })
      }
      ...
      if (settingsData.shippingSetting) {
        setShippingData({
          defaultShippingRate: settingsData.shippingSetting.defaultShippingRate || 50,
          freeShippingThreshold: settingsData.shippingSetting.freeShippingThreshold || 1000,
          shippingZones: settingsData.shippingSetting.shippingZones?.map((zone, index) => ({
            id: index + 1,
            name: zone.zoneName,
          ...
          deliveryTime: {
            standard: settingsData.shippingSetting.deliveryTimeframes?.standard || '3-6 business days',
            express: settingsData.shippingSetting.deliveryTimeframes?.express || '1-2 business days',
            overnight: settingsData.shippingSetting.deliveryTimeframes?.overnight || 'Next day delivery'
          }
        })
      }
    }
          ...
          updateData = {
            seoSetting: {
              basicSetting: {
                ...
                keywords: seoData.siteKeywords.split(',').map(k => k.trim()).filter(k => k),
          ...
          break
                    ...
                    case 'profile':
                      ...
                      if (currentUser?._id) {
                        ...
                        const success = await dispatch(updateProfile(currentUser._id, {
                    ...
                    case 'security':
                      ...
                      console.log('Security settings saved using user API')
                      if (currentUser?._id) {
                        console.log('Updating security settings for user:', currentUser._id)
                        ...
                        console.log('Security update result:', success)
  ...
  const handleEndAllOtherSessions = async () => {
    try {
      if (currentUser?._id && sessions.length > 0) {
        ...
        if (currentSessionId) {
          const success = await dispatch(endAllOtherSessions(currentUser._id, currentSessionId))
          if (success) {
  ...
  const handlePasswordChange = async (currentPassword: string, newPassword: string): Promise<boolean> => {
    try {
      console.log('Changing password using resetPassword API...')
      if (!currentUser?._id) {
  ...
  const handleAIPasswordChange = async () => {
    try {
      ...
      console.log('AI password change initiated')
  ...
  // Debug current user
  console.log('Settings page - currentUser:', currentUser)
  console.log('Settings page - userLoading:', userLoading)
