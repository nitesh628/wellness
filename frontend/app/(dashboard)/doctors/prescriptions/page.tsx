...
import { 
  FileText, Plus, Search, Eye, Trash2, Download, Loader2, 
  Pill, CheckCircle, Package
} from 'lucide-react'
...
import { fetchProductsData, selectProductsData } from '@/lib/redux/features/productSlice' 
...
const PrescriptionsPage = () => {
  ...
  // Initial Fetch
  useEffect(() => {
    dispatch(fetchPrescriptions({ page: 1, status: statusFilter, search: searchTerm }))
  ...
  const addProductToPrescription = (product: any) => {
    const newMedication: PrescriptionMedication = {
      ...
      dosage: product.weightSize?.value ? `${product.weightSize.value}${product.weightSize.unit}` : '500mg',
            ...
            <div className="flex flex-col lg:flex-row gap-4">
              ...
              <div className="flex gap-2">
                 ...
                 <div className="flex border rounded-lg overflow-hidden">
                    <Button variant={viewMode === 'prescriptions' ? 'default' : 'ghost'} size="icon" onClick={() => setViewMode('prescriptions')}><FileText className="w-4 h-4" /></Button>
                    <Button variant={viewMode === 'products' ? 'default' : 'ghost'} size="icon" onClick={() => setViewMode('products')}><Pill className="w-4 h-4" /></Button>
                 </div>
              </div>
            </div>
          </CardContent>
                    ...
                    <TableBody>
                        {prescriptions.map((prescription) => (
                            <TableRow key={prescription._id}>
                                ...
                                <TableCell><Badge variant={getStatusColor(prescription.status) as any}>{prescription.status}</Badge></TableCell>
                                <TableCell>
                                    <div className="flex items-center gap-2">
                                        <Button variant="ghost" size="sm" onClick={() => router.push(`/prescriptions/${prescription._id}`)}><Eye className="w-4 h-4" /></Button>
                                        <Button variant="ghost" size="sm" onClick={() => handleDeletePrescription(prescription._id)}><Trash2 className="w-4 h-4 text-red-500" /></Button>
                                    </div>
                                </TableCell>
                            </TableRow>
                        ))}
                  ...
                  {products.map((product: any) => (
                      <Card key={product._id}>
                          <CardContent className="p-6">
                              ...
                              <p className="text-sm text-muted-foreground mb-4">Price: ₹{product.price?.amount}</p>
                              <Button className="w-full" variant="outline" size="sm" onClick={() => {
                                  setIsAddModalOpen(true);
                                  addProductToPrescription(product);
                              }}>Create Prescription with this</Button>
                          </CardContent>
                      </Card>
                  ))}
               </div>
        ...
        <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
          <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
                ...
                <TabsContent value="details" className="space-y-4 py-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <Input value={patientId} onChange={e => setPatientId(e.target.value)} placeholder="User ID (from DB)" />
                        </div>
                        <div>
                            ...
                            <div className="space-y-2 h-64 overflow-y-auto">
                                {products.map((p: any) => (
                                    <div key={p._id} className="flex justify-between items-center border p-2 rounded">
                                        <span>{p.name}</span>
                                        <Button size="sm" onClick={() => addProductToPrescription(p)}><Plus className="w-4 h-4" /></Button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div className="border p-4 rounded-lg">
                            ...
                            <div className="space-y-4 h-64 overflow-y-auto">
                                {selectedProducts.map((med, idx) => (
                                    <div key={idx} className="border p-3 rounded bg-muted/20">
                                        <div className="flex justify-between mb-2">
                                            <span className="font-bold">{med.productName}</span>
                                            <Button size="sm" variant="ghost" onClick={() => removeProductFromPrescription(idx)}><Trash2 className="w-4 h-4 text-red-500" /></Button>
                                        </div>
                                        <div className="grid grid-cols-2 gap-2">
                                            <Input value={med.dosage} onChange={e => updateMedication(idx, 'dosage', e.target.value)} placeholder="Dosage" className="h-8 text-xs" />
                                            <Input value={med.frequency} onChange={e => updateMedication(idx, 'frequency', e.target.value)} placeholder="Freq" className="h-8 text-xs" />
                                            <Input value={med.duration} onChange={e => updateMedication(idx, 'duration', e.target.value)} placeholder="Duration" className="h-8 text-xs" />
                                            <Input type="number" value={med.quantity} onChange={e => updateMedication(idx, 'quantity', Number(e.target.value))} placeholder="Qty" className="h-8 text-xs" />
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            <DialogFooter>
                ...
                <Button onClick={handleAddPrescription} disabled={isLoading}>{isLoading ? 'Saving...' : 'Create'}</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
